...
void SceneBlend::Update(double dt)
{
	HandleKeyPress();
...
void SceneBlend::RenderMesh(Mesh* mesh, bool enableLight)
{
...
void SceneBlend::RenderSkybox()
{
	modelStack.PushMatrix();
	modelStack.Translate(0.f, 0.f, -50.f); // SO THAT THE QUAD IS OFF BY 50 UNITS
	RenderMesh(meshList[GEO_FRONT], false); // SKYBOX SHOULD BE RENDERED WITHOUT
	
	modelStack.PopMatrix();
}


void SceneBlend::Exit()
{
	// Cleanup VBO here
	for (int i = 0; i < NUM_GEOMETRY; ++i)
	{
...
void SceneBlend::HandleKeyPress()
{
